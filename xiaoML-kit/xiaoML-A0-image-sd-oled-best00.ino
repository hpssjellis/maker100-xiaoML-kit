...
// Counter for display
int myOledCount = 0;
int imageCount = 0; // actually for the filename
unsigned long myPreviousTime = 0;
const long myWaitTime = 3000;
...
// Check if the frame buffer was successfully captured
if (fb) {

  // Print the complete string to the serial monitor
  Serial.println("Frame buffer size: " + String(fb->len / 1024.0) + " Kb");
...
void setup() {
  ...
  while (millis() - myPreviousTime <= myWaitTime) {
   // wait for camera to self check brightness
   // to reduce JPG size
   // Main reason for camera fail
   delay(10);
  }
